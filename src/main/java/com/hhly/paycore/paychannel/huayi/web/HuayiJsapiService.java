 ...
 * @version 1.0
 */
public class HuayiJsapiService extends PayAbstract {
	private static Logger logger = Logger.getLogger(HuayiJsapiService.class);

	public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
		logger.info("华移公众号支付开始，请求参数：" + paymentInfo.toString());
		try {
			// 构建扫码请求参数
			Map<String, String> map = HuayiUtil.buildWxScanCodeParam(paymentInfo);
			logger.info("华移公众号支付请求参数：" + map.toString());
			// 如果是微信支付
			String resultJson = HttpUtil.doPost(HuayiConfig.HUAYI_REQUEST_URL + HuayiUtil.WECHART_JSAPI_URL, map);
			logger.info("华移公众号支付返回：" + resultJson);
