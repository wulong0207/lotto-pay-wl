 ...
 * @company 益彩网络科技有限公司
 */
public class KjPayJsApiService extends PayAbstract {

    private static Logger logger = Logger.getLogger(KjPayJsApiService.class);


    public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
        logger.info("快接公众号支付开始，请求参数：" + paymentInfo.toString());
        try {
            // 构建扫码请求参数
            Map<String, String> map = KjPayUtil.buildWxScanCodeParam(paymentInfo);
            logger.info("快接公众号支付请求参数：" + map.toString());
            String payUrl = KjConfig.KJ_PAY_URL;
            // 如果是微信支付
            payUrl = payUrl + KjPayUtil.WECHAR + KjPayUtil.PUBLIC_URL;

            String resultJson = HttpUtil.doPost(payUrl, map);
            logger.info("快接公众号支付返回：" + resultJson);
