 ...
 * @company 益彩网络科技有限公司
 */
public class KjPayWapService extends PayAbstract {

    private static Logger logger = Logger.getLogger(KjPayWapService.class);


    public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
        logger.info("快接wap支付开始，请求参数：" + paymentInfo.toString());
        try {
            // 构建扫码请求参数
