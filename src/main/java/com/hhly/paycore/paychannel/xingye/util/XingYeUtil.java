package com.hhly.paycore.paychannel.xingye.util;
 ...
 * @version 1.0
 */
public class XingYeUtil {
	private static final Logger logger = Logger.getLogger(XingYeUtil.class);
	...
	*/
	...
	public static ResultBO<Map<String, String>> pay(PaymentInfoBO paymentInfo, String platform) {
		...
		logger.info("兴业【" + payType.getValue() + "】支付请求参数：" + paymentInfo.toString());
		String[] mchInfo = getMchInfo(paymentInfo.getPayType(), platform);
		String key = mchInfo[0];
		String mchId = mchInfo[1];
		String interfaceType = mchInfo[2];

		Map<String, String> paramMap = buildPayParam(paymentInfo, mchId, key, interfaceType);
		// 调用接口
		Map<String, String> resultMap = HttpUtil.doPostXml(paramMap, XingYeConfig.XY_PAY_URL);
		logger.info("兴业请求支付返回：" + resultMap.toString());
		ResultBO<?> resultBO = checkPayResult(paymentInfo, payType, key, resultMap);
		if (resultBO.isError()) {
	...
	*/
	public static Map<String, String> buildPayParam(PaymentInfoBO paymentInfo, String mchId, String key, String interfaceType) {
		try {
			// 装载订单信息
			...
			if (paymentInfo.getPayType().equals(PayConstants.PayTypeThirdEnum.ALIPAY_PAYMENT.getKey())) {
				// 支付宝wap支付，需要用户的支付宝账号或者用户支付宝的用户Id，这二个参数在目前是获取不到的
				if (interfaceType.equals(PayConstants.XingYeTradeTypeEnum.ALIPAY_WAP.getValue())) {
					map.put("buyer_logon_id", "xjg-280@163.com");// 买家支付宝账号，和buger_id不能同时为空
					// map.put("buyer_id", paymentInfo.getUserId());// 买家支付宝用户ID
				}
			} else {
			...
			// 下面参数可选
			// map.put("limit_credit_pay", "0");// 限定用户使用时能否使用信用卡，值为1，禁用信用卡，值为0或者不传此参数则不禁用
			// map.put("device_info", "");// 终端设备号
			// map.put("op_user_id", "");//操作员帐号,默认为商户号
			// map.put("goods_tag", "");//商品标记，用于优惠券或者满减使用
			// map.put("product_id", "");//预留字段此 id 为静态可打印的二维码中包含的商品 ID，商户自行维护
	...
	* @param key
	* @param interfaceType
	* @time: 2017年11月23日 上午10:42:48
	* @return: Map<String,String> 
	*/
	public static Map<String, String> buildWXPayParam(PaymentInfoBO paymentInfo, String mchId, String key, String interfaceType) {
		try {
			// 装载订单信息
			Map<String, String> map = new HashMap<String, String>();
			map.put("service", interfaceType);// 接口类型
			...
			map.put("total_fee", GetWeChatUtil.getMoney(paymentInfo.getMoneyOrder()));// 总金额，以分为单位，不允许包含任何字、符号
			map.put("notify_url", paymentInfo.getNotifyUrl());// 通知地址
			map.put("mch_create_ip", paymentInfo.getUserreqIp());// 客户端请求IP
			map.put("nonce_str", GetWeChatUtil.getRandomStr(32));// 随机字符串，不长于 32 位
	...
	/**  
	* 方法说明: 根据支付宝、微信、手Q获取各自的商户号和密钥【key,mchId,interfaceType,refundUrl】
	* @auth: xiongJinGang
	* @param rechargeChannel
	* @time: 2017年10月13日 上午11:19:25
	* @return: void 
	*/
	public static String[] getMchInfo(String rechargeChannel, String platform) {
		...
		case ALIPAY_PAYMENT:// 支付宝支付
			str[0] = XingYeConfig.XY_ALI_PAY_KEY;
			str[1] = XingYeConfig.XY_ALI_PAY_MCH_ID;
			...
			str[3] = XingYeConfig.XY_REFUND_URL;// 退款地址
			break;
		case WEIXIN_PAYMENT:// 微信支付
			str[0] = XingYeConfig.XY_WX_PAY_KEY;
			str[1] = XingYeConfig.XY_WX_PAY_MCH_ID;
		...
		case QQ_PAYMENT:// QQ钱包支付
			if (platform.equals(PayAbstract.PLATFORM_WEB)) {
				str[0] = XingYeConfig.XY_QQ_PAY_KEY;
				str[1] = XingYeConfig.XY_QQ_PAY_MCH_ID;
				str[2] = PayConstants.XingYeTradeTypeEnum.QQ_SCANCODE.getValue();
				str[3] = XingYeConfig.XY_QQ_REFUND_URL;// 退款地址
			} else {
				str[0] = XingYeConfig.XY_QQ_WALLET_PAY_KEY;
				str[1] = XingYeConfig.XY_QQ_WALLET_PAY_MCH_ID;
	...
	public static ResultBO<?> analyPayResult(Map<String, String> map) {
		...
		if (map.containsKey("sign")) {
			...
			if (!map.containsKey("trade_type")) {
				logger.info("兴业扫码支付异步通知trade_type为空");
				return ResultBO.err();
			}
			String key = null;
			...
			case QQ_SCANCODE2:// QQ扫码，支付完成，异步通知用的是这个
				key = XingYeConfig.XY_QQ_PAY_KEY;
				break;
			case QQ_WAP:
				key = XingYeConfig.XY_QQ_WALLET_PAY_KEY;
				break;
			case ALIPAY_SCANCODE:
				key = XingYeConfig.XY_ALI_PAY_KEY;
				break;
			case ALIPAY_WAP:
				key = XingYeConfig.XY_ALI_PAY_KEY;
				break;
			case WX_WAP:
				key = XingYeConfig.XY_WX_PAY_KEY;
				break;
			default:
				break;
			}
	...
	* @return: ResultBO<?> 
	*/
	private static ResultBO<?> checkPayResult(PaymentInfoBO paymentInfo, PayTypeThirdEnum payType, String key, Map<String, String> resultMap) {
		if (!ObjectUtil.isBlank(resultMap)) {
			// 0表示成功，非0表示失败此字段是通信标识
			if (PayAbstract.TRADE_SUCCESS.equals(resultMap.get("status"))) {
				if (PayAbstract.TRADE_SUCCESS.equals(resultMap.get("result_code"))) {
					if (resultMap.containsKey("sign")) {
						if (!SignUtils.checkParam(resultMap, key)) {
