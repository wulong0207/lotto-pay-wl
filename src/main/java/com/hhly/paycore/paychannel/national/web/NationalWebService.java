 ...
 * @version 1.0
 */
public class NationalWebService extends PayAbstract {
	private static Logger logger = Logger.getLogger(NationalWebService.class);

	public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
		logger.info("国连web支付开始，请求参数：" + paymentInfo.toString());
		try {
			// 构建扫码请求参数
			Map<String, String> map = NationalUtil.buildWapMapParam(paymentInfo, PLATFORM_WEB);
