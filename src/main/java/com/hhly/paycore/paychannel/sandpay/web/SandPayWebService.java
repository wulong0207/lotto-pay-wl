...
/**
 * @desc 六度支付(深圳六度方寸技术有限公司)
 * @author xiongJinGang
 * @date 2018年6月13日
 * @company 益彩网络科技公司
 * @version 1.0
 */
public class SandPayWebService extends PayAbstract {
	private static Logger logger = Logger.getLogger(SandPayWebService.class);

	public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
		logger.info("六度wap支付开始，请求参数：" + paymentInfo.toString());
		try {
			// 如果是QQ钱包支付
