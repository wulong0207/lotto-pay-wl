...
public class WeifutongJsapiService extends PayAbstract {
	...
	public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
		ResultBO<Map<String, String>> resultBO = WFTZhongxin2Util.pay(paymentInfo, PLATFORM_JSAPI);
		if (resultBO.isError()) {
			return resultBO;
		}
	...
	public ResultBO<?> refund(RefundParamVO refundParam) {
		return WFTZhongxin2Util.orderRefund(refundParam, PLATFORM_JSAPI);
	}

	@Override
	public ResultBO<?> payQuery(PayQueryParamVO payQueryParamVO) {
		return WFTZhongxin2Util.queryResult(payQueryParamVO, PLATFORM_JSAPI);
	}

	@Override
	public ResultBO<?> refundQuery(PayQueryParamVO payQueryParamVO) {
		return WFTZhongxin2Util.refundQuery(payQueryParamVO, PLATFORM_JSAPI);
	}
