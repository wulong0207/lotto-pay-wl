...
public class WeifutongWebService extends PayAbstract {
	...
	public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
		ResultBO<Map<String, String>> resultBO = WFTZhongxin1Util.pay(paymentInfo, PLATFORM_WEB);
		if (resultBO.isError()) {
			return resultBO;
		}
	...
	public ResultBO<?> refund(RefundParamVO refundParam) {
		return WFTZhongxin1Util.orderRefund(refundParam, PLATFORM_WEB);
	}

	@Override
	public ResultBO<?> payQuery(PayQueryParamVO payQueryParamVO) {
		return WFTZhongxin1Util.queryResult(payQueryParamVO, PLATFORM_WEB);
	}

	@Override
	public ResultBO<?> refundQuery(PayQueryParamVO payQueryParamVO) {
		return WFTZhongxin1Util.refundQuery(payQueryParamVO, PLATFORM_WEB);
	}
