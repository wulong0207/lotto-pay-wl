package com.hhly.paycore.paychannel.weifutong.zxsz2.util;
...
public class WFTZhongxin2Util {
	private static final Logger logger = Logger.getLogger(WFTZhongxin2Util.class);
	...
	*/
	...
	public static ResultBO<Map<String, String>> pay(PaymentInfoBO paymentInfo, String platform) {
		...
		logger.info("威富通中信二【" + payType.getValue() + "】支付请求参数：" + paymentInfo.toString());
		String[] mchInfo = WFTZhongxin2Util.getMchInfo(paymentInfo.getPayType(), platform);
		...
		logger.info("威富通中信二请求参数：" + paramMap.toString());
		// 调用接口
		Map<String, String> resultMap = HttpUtil.doPostXml(paramMap, WFTZhongxin2Config.WEIFUTONG_ZXSZ2_TRADE_URL);
		logger.info("威富通中信二请求支付返回：" + resultMap);
		logger.info("威富通中信二请求支付返回：" + resultMap.toString());
	...
	public static Map<String, String> buildPayParam(PaymentInfoBO paymentInfo, String mchId, String key, String interfaceType, String platform) {
		try {
			...
			if (paymentInfo.getPayType().equals(PayConstants.PayTypeThirdEnum.WEIXIN_PAYMENT.getKey())
					&& (platform.equals(PayAbstract.PLATFORM_WAP) || platform.equals(PayAbstract.PLATFORM_APP) || platform.equals(PayAbstract.PLATFORM_JSAPI))) {
	...
	*/
	public static ResultBO<?> queryResult(PayQueryParamVO payQueryParamVO, String platform) {
		...
		paramMap.put("service", PayConstants.XingYeTradeTypeEnum.TRADE_QUERY.getValue());
		...
		if (!ObjectUtil.isBlank(resultMap)) {
			logger.info("查询威富通中信二支付结果返回：" + resultMap.toString());
	...
	public static ResultBO<?> analyPayResult(Map<String, String> map) {
		...
		if (map.containsKey("sign")) {
			...
			if (!map.containsKey("trade_type")) {
				logger.info("威富通中信二支付异步通知trade_type为空");
				return ResultBO.err();
			}
			String key = null;
			...
			case QQ_SCANCODE2:// QQ扫码，支付完成，异步通知用的是这个
				key = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_QQ_PAY_KEY;
				break;
			case QQ_WAP:
				key = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_QQ_WALLET_PAY_KEY;
				break;
			case ALIPAY_SCANCODE:
				key = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_ALI_PAY_KEY;
				break;
			case ALIPAY_WAP:
				// key = WFTGuangdaConfig.XY_ALI_PAY_KEY;
				break;
			case WX_WAP:
				key = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_WAP_PAY_KEY;
				break;
			case WX_APP:
				key = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_APP_PAY_KEY;
				break;
			case WX_SCANCODE:
				key = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_PAY_KEY;
				break;
			case WX_JSAPI:
				key = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_JSAPI_PAY_KEY;
				break;
			default:
				break;
			}
			...
			logger.info("威富通中信二支付异步通知状态失败，不进行处理");
		}
		return ResultBO.err();
	}
	...
	*/
	public static ResultBO<?> orderRefund(RefundParamVO refundParam, String platform) {
		...
		paramMap.put("service", PayConstants.XingYeTradeTypeEnum.REFUND.getValue());// 接口类型：unified.trade.refund
	...
	*/
	public static ResultBO<?> refundQuery(PayQueryParamVO payQueryParamVO, String platform) {
		...
		// 封装请求参数
		Map<String, String> paramMap = new HashMap<String, String>();
		paramMap.put("service", PayConstants.XingYeTradeTypeEnum.REFUND_QUERY.getValue());// 接口类型：unified.trade.refundquery
		paramMap.put("version", "2.0");// 版本号，version默认值是2.0
		paramMap.put("charset", "UTF-8");// 可选值 UTF-8 ，默认为 UTF-8
		paramMap.put("sign_type", "MD5");// 签名类型，取值：MD5默认：MD5
		paramMap.put("mch_id", mchId);
			...
			if (resultMap.containsKey("sign")) {
				...
				if (resultMap.get("status").equals("0") && resultMap.get("result_code").equals("0")) {
					...
					for (int i = 0; i < refundCountInt; i++) {
						...
						logger.info("【" + payQueryParamVO.getTransCode() + "】第" + (i + 1) + "条退款记录：orderRefundNo=" + orderRefundNo + "，refundChannel=" + refundChannel + "，refundId=" + refundId + "，refundFee="
								+ refundFee + "，couponRefund_fee=" + couponRefund_fee + "，refundStatus=" + refundStatus);
	...
	*/
	...
	public static String[] getMchInfo(String rechargeChannel, String platform) {
		...
		case ALIPAY_PAYMENT:// 支付宝支付
			str[0] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_ALI_PAY_KEY;
			str[1] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_ALI_PAY_PARTNER_CODE;
			if (platform.equals(PayAbstract.PLATFORM_WEB)) {
				str[2] = PayConstants.XingYeTradeTypeEnum.ALIPAY_SCANCODE.getValue();
			} else {
				str[2] = PayConstants.XingYeTradeTypeEnum.ALIPAY_WAP.getValue();
			}
			str[3] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_TRADE_URL;// 退款地址
			break;
		case WEIXIN_PAYMENT:// 微信支付
			if (platform.equals(PayAbstract.PLATFORM_WAP)) {
				str[0] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_WAP_PAY_KEY;
				str[1] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_WAP_PAY_PARTNER_CODE;
				str[2] = PayConstants.XingYeTradeTypeEnum.WX_WAP.getValue();
			} else if (platform.equals(PayAbstract.PLATFORM_APP)) {
				str[0] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_APP_PAY_KEY;
				str[1] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_APP_PAY_PARTNER_CODE;
				str[2] = PayConstants.XingYeTradeTypeEnum.WX_APP.getValue();
			} else if (platform.equals(PayAbstract.PLATFORM_JSAPI)) {
				str[0] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_JSAPI_PAY_KEY;
				str[1] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_JSAPI_PAY_PARTNER_CODE;
				str[2] = PayConstants.XingYeTradeTypeEnum.WX_JSAPI.getValue();
			} else {
				str[0] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_PAY_KEY;
				str[1] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_WX_PAY_PARTNER_CODE;
				str[2] = PayConstants.XingYeTradeTypeEnum.WX_SCANCODE.getValue();
			}
			str[3] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_TRADE_URL;// 退款地址
			break;
		case QQ_PAYMENT:// QQ钱包支付
			if (platform.equals(PayAbstract.PLATFORM_WEB)) {
				str[0] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_QQ_PAY_KEY;
				str[1] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_QQ_PAY_PARTNER_CODE;
				str[2] = PayConstants.XingYeTradeTypeEnum.QQ_SCANCODE.getValue();
			} else if (platform.equals(PayAbstract.PLATFORM_WAP)) {
				str[0] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_QQ_WALLET_PAY_KEY;
				str[1] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_QQ_WALLET_PAY_PARTNER_CODE;
				str[2] = PayConstants.XingYeTradeTypeEnum.QQ_WAP.getValue();
			}
			str[3] = WFTZhongxin2Config.WEIFUTONG_ZXSZ2_TRADE_URL;// 退款地址
			break;
		default:
	...
	*/
	private static ResultBO<?> checkPayResult(PaymentInfoBO paymentInfo, PayTypeThirdEnum payType, String key, Map<String, String> resultMap, String platform) {
		if (!ObjectUtil.isBlank(resultMap)) {
			...
			if (paymentInfo.getPayType().equals(PayConstants.PayTypeThirdEnum.WEIXIN_PAYMENT.getKey()) && platform.equals(PayAbstract.PLATFORM_APP)) {
				isCheckCode = false;
			}
