 ...
 * @version 1.0
 */
public class WeifutongWapService extends PayAbstract {

	public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
		ResultBO<Map<String, String>> resultBO = WFTZhongxin2Util.pay(paymentInfo, PLATFORM_WAP);
		if (resultBO.isError()) {
			return resultBO;
		}
	...
	public ResultBO<?> refund(RefundParamVO refundParam) {
		return WFTZhongxin2Util.orderRefund(refundParam, PLATFORM_WAP);
	}

	@Override
	public ResultBO<?> payQuery(PayQueryParamVO payQueryParamVO) {
		return WFTZhongxin2Util.queryResult(payQueryParamVO, PLATFORM_WAP);
	}

	@Override
	public ResultBO<?> refundQuery(PayQueryParamVO payQueryParamVO) {
		return WFTZhongxin2Util.refundQuery(payQueryParamVO, PLATFORM_WAP);
	}
