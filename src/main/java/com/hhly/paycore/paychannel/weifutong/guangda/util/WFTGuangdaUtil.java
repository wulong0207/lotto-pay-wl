package com.hhly.paycore.paychannel.weifutong.guangda.util;
 ...
 * @version 1.0
 */
public class WFTGuangdaUtil {
	private static final Logger logger = Logger.getLogger(WFTGuangdaUtil.class);
	...
	*/
	...
	public static ResultBO<Map<String, String>> pay(PaymentInfoBO paymentInfo, String platform) {
		...
		logger.info("威富通光大【" + payType.getValue() + "】支付请求参数：" + paymentInfo.toString());
		String[] mchInfo = WFTGuangdaUtil.getMchInfo(paymentInfo.getPayType(), platform);
		...
		// 调用接口
		Map<String, String> resultMap = HttpUtil.doPostXml(paramMap, WFTGuangdaConfig.WEIFUTONG_GUANGDA_TRADE_URL);
		logger.info("威富通光大请求支付返回：" + resultMap.toString());
	...
	*/
	public static Map<String, String> buildPayParam(PaymentInfoBO paymentInfo, String mchId, String key, String interfaceType, String platform) {
		try {
			...
			// 微信支付的wap支付，需要这些参数
			if (paymentInfo.getPayType().equals(PayConstants.PayTypeThirdEnum.WEIXIN_PAYMENT.getKey()) && (platform.equals(PayAbstract.PLATFORM_WAP) || platform.equals(PayAbstract.PLATFORM_APP) || platform.equals(PayAbstract.PLATFORM_JSAPI))) {
	...
	*/
	public static ResultBO<?> queryResult(PayQueryParamVO payQueryParamVO, String platform) {
		...
		if (!ObjectUtil.isBlank(resultMap)) {
			logger.info("查询威富通光大支付结果返回：" + resultMap.toString());
	...
	public static ResultBO<?> analyPayResult(Map<String, String> map) {
		...
		if (map.containsKey("sign")) {
			...
			if (!map.containsKey("trade_type")) {
				logger.info("威富通光大支付异步通知trade_type为空");
				return ResultBO.err();
			}
			String key = null;
			...
			case QQ_SCANCODE2:// QQ扫码，支付完成，异步通知用的是这个
				key = WFTGuangdaConfig.WEIFUTONG_GUANGDA_QQ_PAY_KEY;
				break;
			case QQ_WAP:
				key = WFTGuangdaConfig.WEIFUTONG_GUANGDA_QQ_WALLET_PAY_KEY;
				break;
			case ALIPAY_SCANCODE:
				key = WFTGuangdaConfig.WEIFUTONG_GUANGDA_ALI_PAY_KEY;
				break;
			case ALIPAY_WAP:
				// key = WFTGuangdaConfig.XY_ALI_PAY_KEY;
				break;
			case WX_WAP:
				key = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_WAP_PAY_KEY;
				break;
			case WX_APP:
				key = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_APP_PAY_KEY;
				break;
			case WX_SCANCODE:
				key = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_PAY_KEY;
				break;
			case WX_JSAPI:
				key = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_JSAPI_PAY_KEY;
				break;
			default:
				break;
			}
			...
			logger.info("威富通光大支付异步通知状态失败，不进行处理");
		}
		return ResultBO.err();
	}
	...
	*/
	...
	public static String[] getMchInfo(String rechargeChannel, String platform) {
		...
		case ALIPAY_PAYMENT:// 支付宝支付
			str[0] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_ALI_PAY_KEY;
			str[1] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_ALI_PAY_PARTNER_CODE;
			...
			str[3] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_TRADE_URL;// 退款地址
			break;
		case WEIXIN_PAYMENT:// 微信支付
			if (platform.equals(PayAbstract.PLATFORM_WAP)) {
				str[0] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_WAP_PAY_KEY;
				str[1] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_WAP_PAY_PARTNER_CODE;
				str[2] = PayConstants.XingYeTradeTypeEnum.WX_WAP.getValue();
			} else if (platform.equals(PayAbstract.PLATFORM_APP)) {
				str[0] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_APP_PAY_KEY;
				str[1] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_APP_PAY_PARTNER_CODE;
				str[2] = PayConstants.XingYeTradeTypeEnum.WX_APP.getValue();
			} else if (platform.equals(PayAbstract.PLATFORM_JSAPI)) {
				str[0] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_JSAPI_PAY_KEY;
				str[1] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_JSAPI_PAY_PARTNER_CODE;
				str[2] = PayConstants.XingYeTradeTypeEnum.WX_JSAPI.getValue();
			} else {
				str[0] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_PAY_KEY;
				str[1] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_WX_PAY_PARTNER_CODE;
		...
		case QQ_PAYMENT:// QQ钱包支付
			if (platform.equals(PayAbstract.PLATFORM_WEB)) {
				str[0] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_QQ_PAY_KEY;
				str[1] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_QQ_PAY_PARTNER_CODE;
				str[2] = PayConstants.XingYeTradeTypeEnum.QQ_SCANCODE.getValue();
			} else if (platform.equals(PayAbstract.PLATFORM_WAP)) {
				str[0] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_QQ_WALLET_PAY_KEY;
				str[1] = WFTGuangdaConfig.WEIFUTONG_GUANGDA_QQ_WALLET_PAY_PARTNER_CODE;
