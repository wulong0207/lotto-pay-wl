...
public class WeifutongWapService extends PayAbstract {
	...
	public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
		ResultBO<Map<String, String>> resultBO = WFTXingYeUtil.pay(paymentInfo, PLATFORM_WAP);
		if (resultBO.isError()) {
			return resultBO;
		}
	...
	public ResultBO<?> refund(RefundParamVO refundParam) {
		return WFTXingYeUtil.orderRefund(refundParam, PLATFORM_WAP);
	}

	@Override
	public ResultBO<?> payQuery(PayQueryParamVO payQueryParamVO) {
		return WFTXingYeUtil.queryResult(payQueryParamVO, PLATFORM_WAP);
	}

	@Override
	public ResultBO<?> refundQuery(PayQueryParamVO payQueryParamVO) {
		return WFTXingYeUtil.refundQuery(payQueryParamVO, PLATFORM_WAP);
	}
