...
public class WeifutongWebService extends PayAbstract {
	...
	public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
		ResultBO<Map<String, String>> resultBO = WFTGuangdaUtil.pay(paymentInfo, PLATFORM_WEB);
		if (resultBO.isError()) {
			return resultBO;
		}
	...
	public ResultBO<?> refund(RefundParamVO refundParam) {
		return WFTGuangdaUtil.orderRefund(refundParam, PLATFORM_WEB);
	}

	@Override
	public ResultBO<?> payQuery(PayQueryParamVO payQueryParamVO) {
		return WFTGuangdaUtil.queryResult(payQueryParamVO, PLATFORM_WEB);
	}

	@Override
	public ResultBO<?> refundQuery(PayQueryParamVO payQueryParamVO) {
		return WFTGuangdaUtil.refundQuery(payQueryParamVO, PLATFORM_WEB);
	}
