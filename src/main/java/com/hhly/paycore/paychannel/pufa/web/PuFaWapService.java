 ...
 * @version 1.0
 */
public class PuFaWapService extends PayAbstract {
	private static Logger logger = Logger.getLogger(PuFaWapService.class);

	public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
		logger.info("浦发WAP开始，请求参数：" + paymentInfo.toString());
		...
		try {
			...
			logger.info("浦发WAP支付请求返回：" + result);
