 ...
 * @version 1.0
 */
public class JuhePayWebService extends PayAbstract {

	@Override
	public ResultBO<?> pay(PaymentInfoBO paymentInfo) {
		ResultBO<?> resultBO = JuhePayUtil.pay(paymentInfo, PLATFORM_WEB);
		if (resultBO.isError()) {
			return resultBO;
		}
		...
		payReqResultVO.setType(PayConstants.PayReqResultEnum.LINK.getKey());
		String codeUrl = juhePayResponseVO.getCodeUrl();
		if (paymentInfo.getPayType().equals(PayConstants.PayTypeThirdEnum.JD_PAYMENT.getKey())) {
			codeUrl = juhePayResponseVO.getPayInfo();// 京东支付的二维码链接
		}
		payReqResultVO.setFormLink(codeUrl);
	...
	public ResultBO<?> payNotify(Map<String, String> map) {
		return JuhePayUtil.payNotify(map, PLATFORM_WEB);
	}

	@Override
	public ResultBO<?> payQuery(PayQueryParamVO payQueryParamVO) {
		return JuhePayUtil.payQuery(payQueryParamVO, PLATFORM_WEB);
	}
