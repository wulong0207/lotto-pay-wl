...
import com.hhly.paycore.service.*;
...
/**
 * @desc 【对外暴露hession接口】 账户钱包服务层（供hession使用）
 * @author xiongJinGang
 * @date 2017年11月9日
 * @company 益彩网络科技公司
 * @version 1.0
 */
@Service("iUserWalletService")
public class UserWalletServiceImpl implements IUserWalletService {
	...
	@Override
	public ResultBO<?> addRedColorAmount2(List<OperateCouponVO> list) throws Exception {
		return addAmountForCms2(list, null);
	}

	@Override
	public ResultBO<?> addRedColorAmountByType2(List<OperateCouponVO> list, Short transType) throws Exception {
		return addAmountForCms2(list, transType);
	}
	...
	* @return: ResultBO<?> 
	*/
	private ResultBO<?> addAmountForCms2(List<OperateCouponVO> list, Short transType) throws Exception {
		if (CollectionUtils.isEmpty(list)) {
			logger.info("彩金list为空，不生成彩金红包,list=" + list);
			return ResultBO.ok();
		}

		// 往operate_coupon表添加记录
		List<OperateCouponPO> poList = new ArrayList<>();
		for (OperateCouponVO vo : list) {
			if (StringUtils.isBlank(vo.getActivityCode())) {
				logger.info("红包【" + vo.getRedCode() + "】的活动编号为空");
			}
			poList.add(new OperateCouponPO(vo));
		}
		int num = operateCouponMapper.insertBatch(poList);
		if (num <= 0) {
			logger.info("往优惠券表添加彩金红包记录失败：" + poList);
			throw new RuntimeException("往优惠券表添加彩金红包记录失败：" + poList);
		}

		// 判断红包类型为彩金红包时
		List<UserRedAddParamVo> voList = list.stream()
				// `red_type` 1：充值红包；2：满减红包；3：彩金红包；4：加奖红包；5：大礼包；6：随机红包',
				// `red_status` '1：待激活；2：待派发；3：可使用；4：已过期；5：已作废；6：已使用',
				.filter(coupon -> coupon.getRedType() == 3 && coupon.getRedStatus().equals("3")).map(coupon -> {
					UserRedAddParamVo vo = new UserRedAddParamVo();
					vo.setUserId(coupon.getUserId());
					vo.setRedAmount(coupon.getRedValue().doubleValue());
					vo.setRedCode(coupon.getRedCode());
					return vo;
		...
		if (!CollectionUtils.isEmpty(voList)) {
			Short splitType = PayConstants.WalletSplitTypeEnum.RED_COLOR.getKey();// 只操作彩金红包账户
			Short operateType = MoneyFlowEnum.IN.getKey();//
			for (UserRedAddParamVo urap : voList) {
				logger.info("给用户【" + urap.getUserId() + "】账户添加彩金参数：" + urap.toString());
				// 1、更新账户钱包中彩金红包金额
				ResultBO<?> resultBO = updateUserWalletBySplit(urap.getUserId(), urap.getRedAmount(), operateType, splitType);
				// 彩金红包发送
	...
	public ResultBO<?> subRedColorAmount(List<UserRedAddParamVo> list) throws Exception {
		...
		for (UserRedAddParamVo urap : list) {
			...
			urap.setStatus(transStatus);
			urap.setTransStatus(transStatus);
			String orderInfo = Constants.RED_REMARK_CMS_CANCEL;
			if (PayConstants.OperateTypeEnum.OVERDUE.getKey().equals(urap.getOperateType())) {
				orderInfo = Constants.RED_OVERDUE_CANCEL;
			}
			urap.setOrderInfo(orderInfo);
	...
	public ResultBO<?> addWinAmountForCMS(UserRedAddParamVo userRedAddParam) throws Exception {
		...
		userRedAddParam.setCashAmount(userRedAddParam.getRedAmount());
		...
		userRedAddParam.setOrderInfo(Constants.OFFICIAL_AWARD);// 官方加奖
	...
	*/
	private ResultBO<?> updateUserWalletBalance(UserWalletBO userWalletBO, Double updateAmount, Double redAmount, Short redOperateType) throws Exception {
		logger.info("更新账户余额前，资金情况【" + userWalletBO.toString() + "】，操作金额【" + updateAmount + "】，彩金金额【" + redAmount + "】");
		...
		// 总现金金额=原有金额+现在充值金额
		Double totalCashAmount = MathUtil.add(userWalletBO.getTotalCashBalance(), updateAmount);
		// 20%总余额=原有20%余额+充值金额拆分后的20%金额
		Double totalTop20Amount = MathUtil.add(userWalletBO.getTop20Balance(), MathUtil.mul(updateAmount, Constants.USER_WALLET_TWENTY_PERCENT));
		// 80%总余额=原有80%余额+充值金额拆分后的80%金额
		Double totalTop80Amount = MathUtil.add(userWalletBO.getTop80Balance(), MathUtil.mul(updateAmount, Constants.USER_WALLET_EIGHTY_PERCENT));
		// 设置最新的金额
		...
		logger.info("更新账户余额后，资金情况【" + userWalletPO.toString() + "】，操作金额【" + updateAmount + "】，彩金金额【" + redAmount + "】");
	...
	*/
	private ResultBO<?> updateSingleWalletBalance(UserWalletBO userWalletBO, Double amount80, Double amount20, Double amountWin, Double amountRed) throws Exception {
		...
		UserWalletPO userWalletPO = new UserWalletPO(userWalletBO);
		userWalletPO.setTop20Balance(MathUtil.add(amount20, userWalletBO.getTop20Balance()));
		userWalletPO.setTop80Balance(MathUtil.add(amount80, userWalletBO.getTop80Balance()));
		userWalletPO.setWinningBalance(MathUtil.add(amountWin, userWalletBO.getWinningBalance()));
		userWalletPO.setEffRedBalance(MathUtil.add(amountRed, userWalletBO.getEffRedBalance()));
		Double totalAddCashBalance = MathUtil.add(amount80, amount20, amountWin);
		userWalletPO.setTotalCashBalance(MathUtil.add(totalAddCashBalance, userWalletBO.getTotalCashBalance()));
	...
	*/
	private ResultBO<?> subBalanceByOperateType(UserWalletBO userWalletBO, Double updateAmount, Double redAmount, Short redOperateType) throws Exception {
		logger.info("减账户余额前，资金情况【" + userWalletBO.toString() + "】，操作金额【" + updateAmount + "】，操作红包【" + redAmount + "】");
		ResultBO<?> resultBO = PayUtil.countTotalAmount(userWalletBO);
		if (resultBO.isError()) {
		...
		int compareResult = MathUtil.compareTo(userWalletBO.getTop20Balance(), updateAmount);
		// 20%中的余额够扣除
		if (compareResult >= 0) {
			userWalletPO.setTop20Balance(MathUtil.sub(userWalletBO.getTop20Balance(), updateAmount));
			userWalletPO.setUse20Balance(updateAmount);// 使用20%金额
		} else {
			// 20%扣完
			userWalletPO.setTop20Balance(0d);
			userWalletPO.setUse20Balance(userWalletBO.getTop20Balance());// 使用20%金额
			// 剩余需要减的金额
			Double needSubBalanceAmount = MathUtil.sub(updateAmount, userWalletBO.getTop20Balance());
			// 原有的80%金额不够减
			if (MathUtil.compareTo(userWalletBO.getTop80Balance(), needSubBalanceAmount) >= 0) {
				userWalletPO.setTop80Balance(MathUtil.sub(userWalletBO.getTop80Balance(), needSubBalanceAmount));
				userWalletPO.setUse80Balance(needSubBalanceAmount);// 使用80%金额
			} else {
				userWalletPO.setTop80Balance(0d);
				userWalletPO.setUse80Balance(userWalletBO.getTop80Balance());
				// 需要扣除中奖账户的金额
				Double needSubWinAmount = MathUtil.sub(needSubBalanceAmount, userWalletBO.getTop80Balance());
				userWalletPO.setWinningBalance(MathUtil.sub(userWalletBO.getWinningBalance(), needSubWinAmount));// 最新的中奖金额
		...
		if (!ObjectUtil.isBlank(redAmount)) {
			// 红包是进账
			if (redOperateType.equals(PayConstants.MoneyFlowEnum.IN.getKey())) {
				userWalletPO.setEffRedBalance(MathUtil.add(userWalletBO.getEffRedBalance(), redAmount));
			} else {
				// 红包是出账
				userWalletPO.setEffRedBalance(MathUtil.sub(userWalletBO.getEffRedBalance(), redAmount));
			}
		}

		userWalletPO.setTotalCashBalance(MathUtil.sub(totalCashBalance, updateAmount));// 最新总现金金额=原现金总金额-需要扣除的金额
		...
		// 这里设置totalAmount金额，是为了兼容前端的版本不做修改，继续保留该字段
		userWalletBO.setTotalAmount(userWalletBO.getTotalCashBalance());// 账户总余额。
		logger.info("减账户余额后，资金情况【" + userWalletPO.toString() + "】，操作金额【" + updateAmount + "】，操作红包【" + redAmount + "】");
		return ResultBO.ok(userWalletPO);
	}

	private ResultBO<?> subWalletBalance(UserWalletBO userWalletBO, Double updateAmount, Short splitType) throws Exception {
		logger.info("减账户余额前，资金情况【" + userWalletBO.toString() + "】，操作金额【" + updateAmount + "】，splitType：" + splitType);
		...
		if (splitType.equals(PayConstants.WalletSplitTypeEnum.EIGHTY_PERCENT.getKey())) {
			...
			if (MathUtil.compareTo(updateAmount, userWalletBO.getTop80Balance()) > 0) {
				throw new RuntimeException("账户余额不足，操作钱包失败！");
			}
			userWalletPO.setTop80Balance(MathUtil.sub(userWalletBO.getTop80Balance(), updateAmount));
			userWalletPO.setTotalCashBalance(MathUtil.sub(userWalletBO.getTotalCashBalance(), updateAmount));// 设置最新的金额
			userWalletPO.setUse80Balance(updateAmount);
		} else if (splitType.equals(PayConstants.WalletSplitTypeEnum.TWENTY_PERCENT.getKey())) {
			// 操作金额大于需要操作的账户字段，返回错误
			if (MathUtil.compareTo(updateAmount, userWalletBO.getTop20Balance()) > 0) {
				throw new RuntimeException("账户余额不足，操作钱包失败！");
			}
			userWalletPO.setTop20Balance(MathUtil.sub(userWalletBO.getTop20Balance(), updateAmount));
			userWalletPO.setTotalCashBalance(MathUtil.sub(userWalletBO.getTotalCashBalance(), updateAmount));// 设置最新的金额
			userWalletPO.setUse20Balance(updateAmount);
		} else if (splitType.equals(PayConstants.WalletSplitTypeEnum.TWENTY_EIGHTY_PERCENT_RATE.getKey())) {
			// 先扣20%，再扣80%
			subTwentyEight(updateAmount, userWalletBO.getTotalCashBalance(), userWalletBO.getTop20Balance(), userWalletBO.getTop80Balance(), userWalletPO);
		} else if (splitType.equals(PayConstants.WalletSplitTypeEnum.WINNING.getKey())) {// 操作中奖金额
			if (MathUtil.compareTo(updateAmount, userWalletBO.getWinningBalance()) > 0) {
				throw new RuntimeException("中奖账户余额不足，操作钱包失败！");
			}
			userWalletPO.setWinningBalance(MathUtil.sub(userWalletBO.getWinningBalance(), updateAmount));
			userWalletPO.setTotalCashBalance(MathUtil.sub(userWalletBO.getTotalCashBalance(), updateAmount));// 设置最新的金额
			...
			// 操作金额大于需要操作的账户字段，返回错误
			if (MathUtil.compareTo(updateAmount, userWalletBO.getEffRedBalance()) > 0) {
				throw new RuntimeException("红包账户余额不足，操作钱包失败！");
			}
			userWalletPO.setEffRedBalance(MathUtil.sub(userWalletBO.getEffRedBalance(), updateAmount));
		} else if (splitType.equals(PayConstants.WalletSplitTypeEnum.WINNING_TWENTY_EIGHTY.getKey())) {
			// 先扣奖金，再扣20%，最后扣80%
			subWinningTwentyEight(updateAmount, userWalletBO.getTotalCashBalance(), userWalletBO.getTop20Balance(), userWalletBO.getTop80Balance(), userWalletBO.getWinningBalance(), oldUpdateAmount, userWalletPO);
		} else if (splitType.equals(PayConstants.WalletSplitTypeEnum.WINNING_EIGHTY_TWENTY.getKey())) {
			// 先扣奖金，再扣80%，最后扣20%
			subWinnginEightTwenty(updateAmount, userWalletBO.getTotalCashBalance(), userWalletBO.getTop20Balance(), userWalletBO.getTop80Balance(), userWalletBO.getWinningBalance(), oldUpdateAmount, userWalletPO);
		} else if (splitType.equals(PayConstants.WalletSplitTypeEnum.TWENTY_EIGHTY_WINNING.getKey())) {
			// 先扣20%，再扣80%，再扣中奖金额
			subTwentyEightWinning(updateAmount, userWalletBO.getTotalCashBalance(), userWalletBO.getTop20Balance(), userWalletBO.getTop80Balance(), userWalletBO.getWinningBalance(), oldUpdateAmount, userWalletPO);
		} else {
		...
		// 这里设置totalAmount金额，是为了兼容前端的版本不做修改，继续保留该字段
		userWalletBO.setTotalAmount(userWalletBO.getTotalCashBalance());// 账户总余额。
		logger.info("减账户余额后，资金情况【" + userWalletPO.toString() + "】，操作金额【" + oldUpdateAmount + "】");
		return ResultBO.ok(userWalletPO);
	}
	...
	* @param oldTop20Balance
	* @param oldTop80Balance
	* @param userWalletPO
	* @throws RuntimeException
	* @time: 2017年12月23日 下午3:28:56
	* @return: void 
	*/
	private void subTwentyEight(Double updateAmount, Double totalCashBalance, Double oldTop20Balance, Double oldTop80Balance, UserWalletPO userWalletPO) throws RuntimeException {
		...
		if (compareResult >= 0) {
			...
			userWalletPO.setTop80Balance(MathUtil.sub(oldTop80Balance, needSub80BalanceAmount));
			// 设置使用金额
	...
	* @param oldTop20Balance
	* @param oldTop80Balance
	* @param oldWinningBalance
	* @param oldUpdateAmount
	* @param userWalletPO
	* @throws RuntimeException
	* @time: 2017年12月23日 下午3:25:29
	* @return: void 
	*/
	private void subWinningTwentyEight(Double updateAmount, Double totalCashBalance, Double oldTop20Balance, Double oldTop80Balance, Double oldWinningBalance, Double oldUpdateAmount, UserWalletPO userWalletPO) throws RuntimeException {
	...
	* @param userWalletPO
	* @throws RuntimeException
	* @time: 2017年12月23日 下午3:24:27
	* @return: void 
	*/
	private void subWinnginEightTwenty(Double updateAmount, Double totalCashBalance, Double oldTop20Balance, Double oldTop80Balance, Double oldWinningBalance, Double oldUpdateAmount, UserWalletPO userWalletPO) throws RuntimeException {
	...
	* @param userWalletPO
	* @throws RuntimeException
	* @time: 2017年12月23日 下午3:17:21
	* @return: void 
	*/
	private void subTwentyEightWinning(Double updateAmount, Double totalCashBalance, Double oldTop20Balance, Double oldTop80Balance, Double oldWinningBalance, Double oldUpdateAmount, UserWalletPO userWalletPO) throws RuntimeException {
		if (MathUtil.compareTo(updateAmount, totalCashBalance) > 0) {
			throw new RuntimeException("账户余额不足，操作钱包失败！");
		}
		// 判断20%中的余额是否大于需要修改的余额，大于直接扣除，否则再扣80%中的
