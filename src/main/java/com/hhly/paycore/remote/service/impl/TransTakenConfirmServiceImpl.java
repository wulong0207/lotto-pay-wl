...
import com.hhly.skeleton.pay.trans.vo.TakenAppVO;
...
/**
 * @desc 【对外暴露hession接口】 提款交易实现类
 * @author xiongjingang
 * @date 2017年3月2日 上午10:51:55
 * @company 益彩网络科技公司
 * @version 1.0
 */
@Service("iTransTakenConfirmService")
public class TransTakenConfirmServiceImpl implements ITransTakenConfirmService {
	...
	public ResultBO<?> findTakenListByPage(TransParamVO transParamVO) throws Exception {
		...
		if (count > 0) {
			...
			// 平台为空或者平台不为空并且是PC端
			if (ObjectUtil.isBlank(transParamVO.getPlatform()) || (!ObjectUtil.isBlank(transParamVO.getPlatform()) && transParamVO.getPlatform().equals(PayConstants.TakenPlatformEnum.WEB.getKey()))) {
				pageBO.setDataList(list);
			} else {
				pageBO.setDataList(getAppTransTakenList(list));
			}
	 ...
	 */
	private List<AppTransUserVO> getAppTransTakenList(List<TransTakenBO> list) {
		...
		if (!ObjectUtil.isBlank(list)) {
			...
			Map<String, List<TakenAppVO>> map = new LinkedHashMap<String, List<TakenAppVO>>();
			TakenAppVO takenAppVO = null;
			for (TransTakenBO transTakenBO : list) {
				takenAppVO = new TakenAppVO(transTakenBO);
				// 交易状态
				UserTransStatusEnum transStatusEnum = PayConstants.UserTransStatusEnum.getEnum(transTakenBO.getTransStatus());
				if (null != transStatusEnum) {
					transTakenBO.setTransStatusName(transStatusEnum.getValue());
				}
				String yearMonth = DateUtil.convertDateToStr(transTakenBO.getCreateTime(), DateUtil.FORMAT_CHINESE_YYYYMM);// 年月
				String yearMonthDate = DateUtil.convertDateToStr(transTakenBO.getCreateTime(), DateUtil.DATE_FORMAT_NO_LINE);// 年月日
				if (yearMonthDate.equals(today)) {
					takenAppVO.setCreateTimeStr(Constants.APP_TRANS_USER_LIST_TODAY + " " + DateUtil.convertDateToStr(transTakenBO.getCreateTime(), DateUtil.TIME));
				} else if (yearMonthDate.equals(yesteday)) {
					takenAppVO.setCreateTimeStr(Constants.APP_TRANS_USER_LIST_YESTEDAY + " " + DateUtil.convertDateToStr(transTakenBO.getCreateTime(), DateUtil.TIME));
				} else if (yearMonthDate.equals(beforeYesteday)) {
					takenAppVO.setCreateTimeStr(Constants.APP_TRANS_USER_LIST_BEFORE_YESTEDAY + " " + DateUtil.convertDateToStr(transTakenBO.getCreateTime(), DateUtil.TIME));
				} else {
					takenAppVO.setCreateTimeStr(DateUtil.convertDateToStr(transTakenBO.getCreateTime(), DateUtil.FORMAT_CHINESE_DAY));
				}
				if (map.containsKey(yearMonth)) {
					List<TakenAppVO> transList = map.get(yearMonth);
					transList.add(takenAppVO);
					map.put(yearMonth, transList);
				} else {
					List<TakenAppVO> appTransList = new ArrayList<TakenAppVO>();
					appTransList.add(takenAppVO);
			...
			if (map.size() > 0) {
				...
				for (Map.Entry<String, List<TakenAppVO>> transMap : map.entrySet()) {
	...
	public ResultBO<?> addTransTaken(TransTakenVO transTakenVO) throws Exception {
		...
		transTakenPO.setCreateBy(userInfo.getRealName());
	...
	*/
	...
	public ResultBO<?> validateTakenCount(TakenReqParamVO takenReqParamVO) throws Exception {
		UserInfoBO userInfo = userUtil.getUserByToken(takenReqParamVO.getToken());
		if (ObjectUtil.isBlank(userInfo)) {
			return ResultBO.err(MessageCodeConstants.TOKEN_LOSE_SERVICE);
		}
		// 判断账号是否经过实名，2018-01-04，提款加实名验证
		if (ObjectUtil.isBlank(userInfo.getRealName()) || ObjectUtil.isBlank(userInfo.getIdCard())) {
			logger.error("账户【" + userInfo.getRealName() + "】、身份证【" + userInfo.getIdCard() + "】未经过实名认证");
	...
	public ResultBO<?> takenCount(TakenReqParamVO takenReqParamVO) throws Exception {
		...
		TakenBankCardVO takenBankCardVO = takenBankList.get(0);
		// 20%未消费完成
		if (MathUtil.compareTo(top20Balance, 0) > 0) {
			...
			if (MathUtil.compareTo(needTakenAmount, winAnd80Balance) > 0) {
				...
				// 原路退回的充值记录
				resultBO = roadRefundRechargeRecord(userId, takenConfirmVO, top20Balance, needSubServiceChargeAmount, takenBankCardVO);
				if (resultBO.isError()) {
	...
	* @return: Double 
	*/
	private ResultBO<?> roadRefundRechargeRecord(Integer userId, TakenConfirmVO takenConfirmVO, Double top20Balance, Double needSubServiceChargeAmount, TakenBankCardVO takenBankCardVO) throws NumberFormatException {
		// 查询充值记录
		...
		// 需要收手续费的部分
		List<TakenAmountInfoVO> takenList = addNeedChargeTaken(list, needSubServiceChargeAmount, top20Balance, takenBankCardVO);
	...
	public ResultBO<?> takenConfirm(TakenReqParamVO takenReqParamVO) throws Exception {
		String token = takenReqParamVO.getToken();
		UserInfoBO userInfoCache = userUtil.getUserByToken(token);
		if (ObjectUtil.isBlank(userInfoCache)) {
			return ResultBO.err(MessageCodeConstants.TOKEN_LOSE_SERVICE);
		}
		Integer userId = userInfoCache.getId();// 用户Id
		UserInfoBO userInfo = userInfoService.findUserInfo(userId);
	...
	public ResultBO<?> takenReqForApp(String token) throws Exception {
		...
		if (!ObjectUtil.isBlank(userInfoBO.getUserPayId()) && !ObjectUtil.isBlank(takenBankList)) {
		...
		if (!takenBankList.isEmpty()) {
			takenUserInfoVO.setBankList(takenBankList);
		} else {
			// H5的不能为空，一定要一个空数组
			if (userInfo.getLoginPlatform().equals(PayConstants.TakenPlatformEnum.WAP.getKey())) {
				takenUserInfoVO.setBankList(takenBankList);
			}
		}
	...
	*/
	private List<TakenAmountInfoVO> addNeedChargeTaken(List<TransRechargeBO> needBackList, Double needSubServiceChargeAmount, Double total20BalanceInWallet, TakenBankCardVO takenBankCardVO) {
		...
		if (!ObjectUtil.isBlank(needBackList)) {
			for (TransRechargeBO transRechargeBO : needBackList) {
				...
				if (MathUtil.compareTo(serviceFee, nowTakenAmount) > 0) {
					...
					// 银行卡为空，并且渠道不是代理充值渠道，要验证银行信息
					if (ObjectUtil.isBlank(payBankBO) && !transRechargeBO.getRechargeChannel().equals(PayConstants.PayChannelEnum.AGENT_RECHARGE.getKey())) {
						// 根据银行卡ID未获取到银行
					...
					if (!ObjectUtil.isBlank(payBankBO)) {
						...
						logger.info("代理充值提现，没有银行卡，直接取用户指定提现的银行卡：" + takenBankCardVO.getBankId());
						takenAmountInfo.setBankName(takenBankCardVO.getBankName());
						takenAmountInfo.setBankId(takenBankCardVO.getBankId());
						takenAmountInfo.setFullBankCard(takenBankCardVO.getFullBankCard());
					}
	...
	* @return: UserWalletPO 
	*/
	public ResultBO<?> updateTakenOperate(TakenRealAmountVO takenRealAmountVO, UserInfoBO userInfoBO, UserWalletBO userWalletBO, TakenReqParamVO takenReqParamVO) throws Exception {
		// 1、添加提款记录
	...
	* @return: list 
	*/
	public List<TransTakenPO> addTransTakenBatch(List<TakenAmountInfoVO> needTakenList, UserInfoBO userInfoBO, TakenReqParamVO takenReqParamVO) throws Exception {
		...
		for (TakenAmountInfoVO takenAmountInfo : needTakenList) {
			...
			if (takenAmountInfo.getStatus().equals(PayConstants.TakenAmountStatusEnum.EXCEPTION.getKey())) {
				...
				if (ObjectUtil.isBlank(transRechargeBO.getBankCardNum())) {// 充值流水中，没有银行卡号，取申请提款时的银行卡号
	...
	*/
	public List<TransUserPO> addTransUserParam(List<TransTakenPO> list, Double realTakenAmount, String channelId, UserWalletBO userWalletBO, UserWalletPO userWalletPO) {
	...
	*/
	@SuppressWarnings("unchecked")
	public void updateTransUser(List<TransTakenPO> list, List<TransTakenBO> takenList) throws Exception {
		if (!ObjectUtil.isBlank(list)) {
			// 给用户退款，通过第一个来判断状态
	...
	* @return: void 
	*/
	public void updateRechargeTakenStatus(List<TakenAmountInfoVO> list) throws Exception {
		if (!ObjectUtil.isBlank(list)) {
