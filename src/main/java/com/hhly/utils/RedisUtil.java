...
public class RedisUtil {
	...
	* @param map
	* @param seconds
	* @time: 2017年12月9日 上午11:54:25
	* @return: void 
	*/
	public void addMap(String key, Map<String, ?> map, Long seconds) {
		try {
			redisTemplate.opsForHash().putAll(key, map);
			redisTemplate.expire(key, seconds, TimeUnit.SECONDS);
		} catch (Exception e) {
			logger.warn(spellString("addObj {0}={1}", key, map, seconds), e);
		}
	}
	...
	* @param key
	* @param hashKey
	* @time: 2017年12月9日 上午11:58:00
	* @return: Object 
	*/
	@SuppressWarnings("unchecked")
	public <T> T getMap(String key, String hashKey, Class<T> clazz) {
		try {
			Object object = redisTemplate.opsForHash().get(key, hashKey);
			return (T) object;
		} catch (Exception e) {
			logger.warn(spellString("addObj {0}={1}", key, hashKey), e);
		}
		return null;
	}
	...
	* @param key
	* @param amount
	* @time: 2017年12月14日 下午3:56:29
	* @return: void 
	*/
	public Double addOrUpdate(String key, Double amount, long second) {
		Double totalAmount = amount;
		try {
			Object value = getObj(key);
			if (!ObjectUtil.isBlank(value)) {
				totalAmount = MathUtil.add((double) value, amount);
				delObj(key);
				addObj(key, totalAmount, second);
			} else {
				addObj(key, amount, second);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
